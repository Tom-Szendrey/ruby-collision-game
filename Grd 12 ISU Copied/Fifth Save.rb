require 'gosu'
#use main menu class
#make and use an endgame screen
...
#change text colour
#fix score (he wants it *4000 to make it easier for him)
...
class Main < Gosu::Window
  ...
  def update
    ...
    if @timer % 5 == 0 and @timer != @secondTimer
      puts 'Maybe make this easier, yes this is a note in a print '
      @secondTimer = @timer
      @difficultyTimer = 200
    end
    ...
    #block 2
    @blockX[2] = self.width/2 - @blockTwo.width/2 + (Math.sin(Time.now.to_f)*(300+ @difficultyTimer))
    @blockY[2] = 10
    #Block one cannot go outside screen
    if @blockX[2] < 0
      @blockX[2] = 0
